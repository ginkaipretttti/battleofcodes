...
export async function GET(request: Request, { params }: { params: Promise<{ gameId: string }> }) {
  try {
    ...
    const games = await sql`
      SELECT g.*, r.total_rounds, r.current_round, r.status as room_status, r.creator_id
      FROM games g
    ...
    // Check if game was abandoned (host left)
    if (game.status === "abandoned") {
      return NextResponse.json({ hostLeft: true })
    }

    // Check if game is completed
    if (game.room_status === "completed" || game.status === "completed") {
    ...
    if (currentRound.length === 0) {
      ...
      if (nextRoundNumber > game.total_rounds) {
        ...
        // Calculate final results and create certificates
        const finalScores = await sql`
          SELECT gs.*, u.fullname
          FROM game_scores gs
          JOIN users u ON gs.user_id = u.id
        ...
        // Update user stats and create certificates
        for (let i = 0; i < finalScores.length; i++) {
          ...
          const placement = i + 1
          if (i === 0) certificateType = "winner"
          else if (i === 1) certificateType = "second"
          else if (i === 2) certificateType = "third"

          // Create certificate
          await sql`
            INSERT INTO certificates (user_id, game_id, placement, certificate_type, player_name, game_date, total_players, points_earned)
            VALUES (${score.user_id}, ${gameId}, ${placement}, ${certificateType}, ${score.fullname}, NOW(), ${finalScores.length}, ${score.total_points})
